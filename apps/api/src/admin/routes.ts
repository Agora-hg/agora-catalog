...
import { asString, redirect, wantsHtml } from '../http.ts'
import {
  getCompany,
  listCompanies,
  listDeleted,
...
import { acceptDadataCandidate, listDadataQueue, parseSuggestions } from './dadata.ts'
import {
  callListPage,
  categoriesPage,
  claimPage,
...
export async function registerAdmin(app: FastifyInstance, ctx: AppContext) {
  ...
  app.post('/admin/companies/:id/dadata/accept', async (req, reply) => {
    const user = await requireAdmin(req, reply, ctx)
    if (!user) return
    try {
      const { id } = req.params as { id: string }
      const updated = await acceptDadataCandidate(ctx.db, id, bodyOf(req))
      if (wantsHtml(req)) return redirect(reply, `/admin/dadata?notice=${encodeURIComponent('Кандидат принят')}`)
      return serializeAdminCompany(updated!)
    } catch (err) {
      return sendError(req, reply, err)
    }
  ...
  app.get('/admin/dadata', async (req, reply) => {
    const user = await requireAdmin(req, reply, ctx)
    if (!user) return
    const [queue, counts] = await Promise.all([listDadataQueue(ctx.db), getNavCounts(ctx.db)])
    const items = queue.map((row) => ({
      company: row.company,
      sources: row.sources,
      suggestions: row.sources.map((source) => ({ sourceId: source.id, items: parseSuggestions(source.payload) })),
    }))
    return reply.type('text/html').send(dadataPage({ user, counts, items, notice: asString(queryOf(req).notice) || undefined }))
  })
