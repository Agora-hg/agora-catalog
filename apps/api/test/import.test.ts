...
import { companies, companySources, rawYandexOrgs } from '@agora/db'
import { runImport } from '../src/import/run.ts'
import { openTestDb, resetDb, testDatabaseUrl } from './helpers.ts'
...
describe('importer on 20-row fixture', { timeout: 60_000, concurrency: 1 }, () => {
  ...
  it('dedups 20 rows into 13 companies and fills staging', async () => {
    const stats = await runImport({ file: fixture20, batch: 'fixture-20', databaseUrl: url })
  ...
  it('does not overwrite a handmade description on update', async () => {
    ...
    await runImport({ file: fixture20, batch: 'fixture-20', databaseUrl: url })
  ...
  it('marks companies missing from a new run as unknown, never inactive', async () => {
    const stats = await runImport({ file: fixtureOne, batch: 'fixture-one', databaseUrl: url })
    assert.equal(stats.created, 0)
