...
/**
 * Нетоварные признаки заведения с Яндекс.Карт.
 *
 * Проверяем ВХОЖДЕНИЕ, а не точное совпадение. Прошлая версия была привязана
 * к `^доступность$`, а в данных ключ называется «Доступность входа на инвалидной
 * коляске» — фильтр его не ловил. В итоге в теги приехали «Доступность входа»
 * (213 компаний), «Парковка для людей с инвалидностью» (120), «Wi-Fi» (39),
 * «Посещение с животными» (72). Это лезло в чипсы карточек, в поисковый вектор
 * и в генерируемые описания: покупатель упаковки видел «Wi-Fi» как категорию товара.
 *
 * «Тара и упаковочные материалы» тоже убрана: она стоит у 663 компаний из 689,
 * то есть не различает ничего и как фильтр бесполезна.
 */
const NON_PRODUCT_FEATURE =
  /(способ оплаты|^доставка$|^самовывоз$|оплат|^акции$|доступност|парковк|лифт|^инн$|пандус|wi-?fi|животн|предварительн|туалет|терминал|наличн|бесконтакт|кредит|^qr|сбп|^продажа$|^тара и упаковочные материалы$)/i
