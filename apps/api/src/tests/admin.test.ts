...
describe('обращения, категории, DaData', () => {
  ...
  it('DaData: показать кандидатов и принять одного', async () => {
    const company = await seedCompany(db, { name: 'МегаПак', slug: 'megapak', phone: '+7 5' })
        ...
        suggestions: [
          {
            ...
            data: {
              ...
              okved: '17.21',
              state: { status: 'ACTIVE' },
              name: { full_with_opf: 'ООО МЕГАПАК' },
            },
          },
        ],
      },
    ...
    assert.match(page.body, /МегаПак/)
    assert.match(page.body, /7707123456/)
    assert.match(page.body, /Принять/)

    const accept = await app.inject({
      method: 'POST',
      url: `/admin/companies/${company.id}/dadata/accept`,
      headers: { 'content-type': 'application/json', cookie },
      payload: { inn: '7707123456' },
    })
    assert.equal(accept.statusCode, 200)
    assert.equal(JSON.parse(accept.body).inn, '7707123456')
    assert.equal(JSON.parse(accept.body).ogrn, '1027700123456')
    assert.equal(JSON.parse(accept.body).egrul_status, 'ACTIVE')
  })
