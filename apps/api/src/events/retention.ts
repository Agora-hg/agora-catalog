...
export async function ensureAggregatesTable(db: Db): Promise<void> {
  await db.execute(AGGREGATES_TABLE_SQL)
  await db.execute(AGGREGATES_UNIQ_SQL)
  await db.execute(AGGREGATES_DAY_IDX_SQL)
  await db.execute(AGGREGATES_EVENT_IDX_SQL)
}
