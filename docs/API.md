...
## Приём форм — обязательно оба формата

`POST /v1/requests`, `POST /v1/requests/{id}/responses`, `POST /v1/companies/{slug}/claims`
принимают **и `application/json`, и `application/x-www-form-urlencoded`**.

Просил TASK-012, и просил правильно. От фронта требуется, чтобы страница работала
с полностью выключенным JS — а нативная отправка `<form>` без JS уходит только
urlencoded. Принимать один JSON значит сломать то самое требование, ради которого
выбирался серверный рендеринг.

Различаются и ответы:

| запрос | ответ |
|---|---|
| `Content-Type: application/json` | `200` + `{ "id": "..." }` |
| `application/x-www-form-urlencoded` | `303 See Other` + `Location: <откуда пришли>?sent=1` |

Без редиректа браузер без JS останется на голом JSON — для человека это выглядит
как поломка сайта.

Ошибки валидации: для JSON — `422` + тело с полями, для формы — `303` обратно
с `?error=<код>`, страница рисует сообщение из query. Ни в каком виде не отдавать
незаполненную форму пустой.

Honeypot-поле и минимальное время заполнения проверяются одинаково в обоих режимах.
